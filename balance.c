...
static void urg_handler(int signo) {
  printf("urg_handler :: signo %d", signo);
...
int create_serversocket(char *node, char *service) {
  ...
  // copiei hints.ai_family = AF_UNSPEC; da linha a cima pra essa
  hints.ai_family = AF_UNSPEC;
  ...
  status = setsockopt(srv_socket, IPPROTO_IPV6, IPV6_V6ONLY,
                      (char *)&sockoptoff, sizeof(sockoptoff));
  if (status < 0) {
...
void b_readlock(void) { a_readlock(0, 0); }

void c_readlock(int group, int channel) {
  a_readlock(((char *)&(grp_channel(common, group, channel))) - (char *)common,
             sizeof(CHANNEL));
}
...
void b_writelock(void) { a_writelock(0, 0); }
...
int setaddress_noexitonerror(struct in_addr *ipaddr, int *port, char *string,
                             int default_port) {
  char *host_string;
  char *port_string;
  struct hostent *hent;
...
void *stream(int arg, int groupindex, int index, char *client_address,
  ...
  for (;;) {
    ...
    if (outbindhost != NULL) {
      ...
      if (bind(sockfd, (struct sockaddr *)&outbind_addr, sizeof(outbind_addr)) <
          0) {
      }
    }
        ...
        if (chn_status(common, groupindex, index) != 0) {
          if (foreground) {
            fprintf(stderr, "connection failed group %d channel %d\n",
                    groupindex, index);
            fprintf(stderr,
                    "%s:%d needs to be enabled manually using balance -i after "
                    "the problem is solved\n",
                    inet_ntoa(serv_addr.sin_addr), ntohs(serv_addr.sin_port));
          } else {
            syslog(LOG_NOTICE, "connection failed group %d channel %d",
                   groupindex, index);
            syslog(LOG_NOTICE,
                   "%s:%d needs to be enabled manually using balance -i after "
                   "the problem is solved",
...
static void usage(void) {
  ...
  fprintf(stderr,
          "  -t sec    specify connect timeout in seconds (default=%d)\n",
          DEFAULTTIMEOUT);
  fprintf(
      stderr,
      "  -T sec    timeout (seconds) for select (0 => never) (default=%d)\n",
      DEFAULTSELTIMEOUT);
  fprintf(stderr, "   !        separates channelgroups (declaring previous to "
                  "be Round Robin)\n");
  fprintf(
      stderr,
    ...
    for (group = 0; group <= MAXGROUPS; group++) {
      for (i = 0; i < grp_nchannels(mycommon, group); i++) {
        fprintf(stderr, "%3d %2d %s:%d:%d\n", group, i,
                ...
                chn_port(mycommon, group, i), chn_maxc(mycommon, group, i));
      }
    }
  }
...
int shell(char *argument) {
  ...
  if (common->release != release || common->subrelease != subrelease) {
    printf("release mismatch, expecting %d.%d, got %d.%d, exiting.\n", release,
           subrelease, common->release, common->subrelease);
    exit(EX_DATAERR);
  }
      ...
      if (mycmp(command, "quit")) {
        ...
        b_readlock();
        {
          ...
          printf("%3s %4s %2s %3s %16s %5s %4s %11s %4s %11s %11s\n", "GRP",
                 "Type", "#", "S", "ip-address", "port", "c", "totalc", "maxc",
                 "sent", "rcvd");
          for (group = 0; group <= MAXGROUPS; group++) {
            for (i = 0; i < grp_nchannels(common, group); i++) {
              printf("%3d %4s %2d %3s %16s %5d %4d %11u %4d %11llu %11llu\n",
                     group, grp_type(common, group) == GROUP_RR ? "RR" : "Hash",
                     i, chn_status(common, group, i) == 1 ? "ENA" : "dis",
                     inet_ntoa(chn_ipaddr(common, group, i)),
                     chn_port(common, group, i), chn_c(common, group, i),
                     chn_tc(common, group, i), chn_maxc(common, group, i),
        ...
        printf("  create <host> <port>           creates a channel in the "
               "current group\n");
        printf("  assign <channel> <host> <port> reassigns a channel in the "
               "current group\n");
        printf("  disable <channel>              disables specified channel in "
               "current group\n");
        printf("  enable <channel>               enables channel in current "
               "group\n");
        printf("  group <group>                  changes current group to "
               "<group>\n");
        printf("  hash                           sets distribution scheme of "
               "current group to Hash\n");
        printf("  help                           prints this message\n");
        printf("  kill                           kills master process and "
               "quits interactive mode\n");
        printf("  maxc <channel> <maxc>          specifies new maxc for "
               "channel of current group\n");
        printf("  mrtg-bytes <grp> <ch>          print bytes in/out in MRTG "
               "format\n");
        printf("  mrtg-conns <grp> <ch>          print total connections in "
               "MRTG format\n");
        printf("  quit                           quit interactive mode\n");
        printf("  reset <channel>                reset all counters of channel "
               "in current group\n");
        printf("  rr                             sets distribution scheme of "
               "current group to Round Robin\n");
        printf("  show                           show all channels in all "
               "groups\n");
          ...
          if ((arg1 = strtok(NULL, " \t\n")) != NULL) {
            if ((arg2 = strtok(NULL, " \t\n")) != NULL) {
              ...
              if (setaddress_noexitonerror(
                      &chn_ipaddr(common, currentgroup,
                                  grp_nchannels(common, currentgroup)),
                      &chn_port(common, currentgroup,
                                grp_nchannels(common, currentgroup)),
                      arg1, getport(arg2))) {
            ...
            if (chn_status(common, currentgroup, chn) != 0) {
              ...
              if ((arg3 = strtok(NULL, " \t\n")) != NULL) {
                if (setaddress_noexitonerror(
                        ...
                        &chn_port(common, currentgroup, chn), arg2,
                        getport(arg3))) {
        ...
        if ((arg1 = strtok(NULL, " \t\n")) != NULL) {
          if ((arg2 = strtok(NULL, " \t\n")) != NULL) {
            ...
            if (mygroup < 0 || mygroup > MAXGROUPS) {
              ...
              if (mychannel < 0 ||
                  mychannel > grp_nchannels(common, currentgroup)) {
                printf("unknown channel\n");
              } else {
                //
...
int main(int argc, char *argv[]) {
  ...
  if (interactive) {
    ...
    if ((common = (COMMON *)shm_malloc(rendezvousfile, sizeof(COMMON))) ==
        NULL) {
      fprintf(stderr, "cannot alloc COMMON struct\n");
      exit(EX_OSERR);
    }
  ...
  for (;;) {
    ...
    if (debugflag) {
      char buf[1024];
      inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)&cli_addr)->sin6_addr), buf,
                1024);
    ...
    for (;;) {
      ...
      for (;;) {
        if (grp_type(common, groupindex) == GROUP_RR) {
          ...
          uindex = hash_fold(
              (unsigned char *)&(((struct sockaddr_in6 *)&cli_addr)->sin6_addr),
              clilen);
          ...
          if (chn_status(common, groupindex, index) == 1 &&
            ...
            if (hashfailover == 1) {
              ...
              if (debugflag)
                fprintf(stderr, "no valid channel in group %d. Failover?\n",
                        groupindex);
              index = -1;
            }
            break;
